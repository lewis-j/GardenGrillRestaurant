...
if (!empty($title_color)) {
	$content_classes[] = $title_color;
}

if(!empty($font_family)) {
	$content_classes[] = $font_family;
}
...
if(!empty($title_custom_color)) {
	$styles[] = "color:{$title_custom_color}";
}
...
if ( '' !== $title && 'separator_no_text' !== $layout ) {
	...
	$content .= '<h4 ' . $styles . ' class="' . esc_attr(implode(' ', $content_classes)) . '">' . $title . '</h4>';
}
...
TEMPLATE;
echo html_entity_decode($separatorHtml);