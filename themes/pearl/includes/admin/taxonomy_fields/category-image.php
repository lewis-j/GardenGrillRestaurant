...
add_action('category_add_form_fields', 'stm_taxonomy_listing_add_field', 10);
add_action('category_edit_form_fields', 'stm_taxonomy_listing_edit_field', 10, 2);
/** Save Custom Field Of Form */
add_action('created_category', 'stm_taxonomy_listing_image_save', 10, 1);
add_action('edited_category', 'stm_taxonomy_listing_image_save', 10, 1);
	...
	function stm_taxonomy_listing_add_field($taxonomy)
	{
		...
		<div class="form-field">
			...
			<div class="stm-choose-listing-image">
				<input
					type="hidden"
					name="pearl_category_image"
					id="pearl_category_image"
					value=""
					size="40"
					aria-required="true"/>
                ...
                jQuery(function ($) {
                    $(".stm-choose-listing-image .button-primary").on('click', function () {
                        var custom_uploader = wp.media({
                            ...
                            $('#pearl_category_image').val(attachment.id);
...
if (!function_exists('stm_taxonomy_listing_edit_field')) {
	function stm_taxonomy_listing_edit_field($tag)
	{
		$default_image_placeholder = get_template_directory_uri() . '/assets/admin/img/default_170x50.gif';
		$default_image = get_template_directory_uri() . '/assets/admin/img/default_170x50.gif';
		$make_custom_order = $current_image = get_term_meta($tag->term_id, 'pearl_category_image', true);
...
if (!function_exists('stm_taxonomy_listing_image_save')) {
	function stm_taxonomy_listing_image_save($term_id)
	{
		if (isset($_POST['pearl_category_image'])) {
			$option_name = 'pearl_category_image';
			$value = sanitize_text_field($_POST['pearl_category_image']);
