...
if (!empty($prev_post)) {
	$pn_posts['prev'] = array(
        ...
        'image' => get_the_post_thumbnail_url($prev_post->ID,'full'),
	...
	<div class="stm_prevnext">
		<?php foreach ($pn_posts as $post_key => $prev_next): ?>
			<?php if (!empty($prev_next)): ?>
				<div class="stm_prevnext__post stm_prevnext__post_<?php echo esc_attr($post_key) ?>" style="background: url(<?php echo html_entity_decode($prev_next['image']); ?>);">
